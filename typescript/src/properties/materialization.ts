...
/** Materializes one flat String mapping into a new canonical Properties document (materialization.rs). */
...
/** Re-parses the generated bytes under the exact target profile (materialization.rs). */
...
/** Properties parse limits derived from one materialization budget (materialization.rs). */
...
/** Rust `char::is_control()`: C0 (U+0000-001F) plus C1 (U+007F-009F) (materialization.rs). */
...
/** Text budget per selected encoding (materialization.rs). */
...
/** Encodes generated text under one selected encoding, emitting the UTF-16 BOM (materialization.rs). */
...
/** Exact fragment encoding without a BOM (materialization.rs). */
...
/** Exact projection closure of the generated document (materialization.rs). */
