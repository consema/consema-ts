 ...
 * vectors (consema-rs/consema-pvce/src/lib.rs). They run once the
...
test('vector pvce.null-vector: null encodes to the frozen bytes', () => {
  // conformance/vectors/v1.json; consema-rs/consema-pvce/src/lib.rs.
...
test('integer sign octets are 0/1/2 and magnitudes are minimal', () => {
  // consema-rs/consema-pvce/src/lib.rs; zero has sign 0 and empty magnitude.
...
/** One String record for the given text: tag, blob length, then the length-prefixed bytes (lib.rs). */
...
test('decode rejects non-canonical zero integer and invalid sign octets', () => {
  // consema-rs/consema-pvce/src/lib.rs: PVCE 1 TAG_INTEGER len 3 sign 1 mag-len 1 0x00.
