 ...
 *  - consema-rs/consema-hcl/src/lexer.rs (codes module) — the thirteen
 *    `hcl.parse.*@1` lexical codes; consema-rs/consema-hcl/src/parser.rs
 *    (codes module) — hcl.parse.block@1 / label@1 / expression@1 /
 *    directive@1 / newline@1 / separator@1 / duplicate-attribute@1
 *  - consema-rs/consema-hcl/src/lib.rs (hcl.parse.encoding@1, fatal
...
// Lexical hcl.parse.*@1 codes (lexer.rs).
...
// Grammar hcl.parse.*@1 codes (parser.rs).
...
// hcl.projection.*@1 codes (projection.rs).
...
// hcl.materialization.*@1 codes (hcl_v1.rs).
...
/** Atomic materialization failure (hcl_v1.rs; RFC 0014 §9). */
 ...
 * frozen fact the vectors compare, the CODE mapping is edit.rs
 * (RFC 0004 §17).
 */
