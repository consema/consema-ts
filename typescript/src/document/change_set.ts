...
export class SourceEdit {
  ...
  /** Replaced old range (lib.rs). */
  oldSpan(): Span {
    return this.#oldSpan;
  }

  /** Range occupied by replacement bytes in the new snapshot (lib.rs). */
  newSpan(): Span {
    return this.#newSpan;
  }

  /** Exact replacement bytes (lib.rs); logically immutable — treat the returned buffer as read-only. */
...
export class NodeMapping {
  ...
  /** New handle when a one-to-one mapping is known (lib.rs). */
  new(): NodeRef | null {
    return this.#new;
  }

  /** Mapping status (lib.rs). */
  status(): NodeMappingStatus {
    return this.#status;
  }

  /** Stable reason for missing or non-trivial mapping (lib.rs). */
  reason(): string | null {
    return this.#reason;
  }
}

/** Complete immutable description of one atomic document transition (lib.rs). */
export class ChangeSet {
   ...
   * validated facts (lib.rs, doc(hidden) in Rust).
   */
  constructor(
  ...
  /** Base snapshot (lib.rs). */
  oldSnapshot(): SnapshotIdentity {
    return this.#oldSnapshot;
  }

  /** Committed snapshot (lib.rs). */
  newSnapshot(): SnapshotIdentity {
    return this.#newSnapshot;
  }

  /** Ordered non-overlapping source edits (lib.rs). */
  sourceEdits(): readonly SourceEdit[] {
    return this.#sourceEdits;
  }

  /** Explicit node mappings (lib.rs). */
  nodeMappings(): readonly NodeMapping[] {
    return this.#nodeMappings;
  }

  /** Operation diagnostics, never written into either Document (lib.rs). */
  diagnostics(): readonly Diagnostic[] {
