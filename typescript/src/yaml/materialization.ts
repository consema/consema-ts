 ...
 * extension constructor contract is selected (RFC 0007 §11).
 */
...
/** A PortableGraph location consumed by YAML materialization (materialization.rs). */
...
/** One graph-input location mapped to one or more generated YAML origins (materialization.rs). */
...
/** Complete deterministic graph-to-YAML provenance multimap (materialization.rs). */
export class GraphMaterializationProvenanceMap {
  ...
  /** Entries in root, canonical-node, and association traversal order (materialization.rs). */
  entries(): readonly GraphMaterializationProvenanceEntry[] {
    return this.#entries;
  }
}

/** Failed graph attempt without a Document or partial output bytes (materialization.rs). */
...
/** Complete exact PortableGraph-to-YAML materialization (materialization.rs). */
...
/** Materializes one complete PortableGraph as a canonical YAML stream (materialization.rs). */
...
function materializeGraphComplete(
  ...
  try {
    ...
    // mismatch (materialization.rs maps the error away).
...
/** scalar_presentation (materialization.rs). */
...
class GraphProvenanceBuilder {
  ...
  /** Registers one new provenance entry (materialization.rs). */
  ...
  /** Appends one alias origin to an existing entry (materialization.rs). */
...
/** Materializes one complete PortableValue into a canonical YAML document (materialization.rs). */
...
function valueGraph(value: PortableValue, limits: MaterializationLimits): Graph {
  ...
  try {
    ...
    // graph_build_failure (materialization.rs): limits map to
...
/**
 * Structural graph equality for the reparse closure (materialization.rs).
