...
解析器和 decoder 严格检查 UTF‑8、长度溢出、非最短 varint、非规范整数/Decimal、容器计数和嵌套深度（TS 无 `unsafe` 机制——「禁止 unsafe」是 Rust 参考实现侧的约束）。Rust 仓的 hardening 证据（`consema-conformance` 的恶意/边界对抗测试清单与计数，以 consema-rs 仓 SECURITY.md 为准）见 consema-rs 仓 SECURITY.md；本仓对应机制为 conformance 门禁（18 套 / 519 cases 全部执行且零失败；聚合 digest `cfd6e296…` 的比对在 runner 读 provisioned manifest 时执行、ts CI 不 provision 该文件即为 documented skip，见 R10/W4-13）、TS-Rust 差分 harness（byte parity / normalized differential / protocol-exchange）与 node --test 测试套件（运行时零第三方依赖）。如果发现崩溃、无界分配或规范绕过，请附最小输入与触发的 capability contract 报告。
...
`toml.1.0@1` 只对完整合法文档形成 snapshot，非法输入返回 `FatalFormationFailure`。`toml-lang/toml-test v2.2.0` 的 205 个 valid 和 474 个 invalid TOML 1.0 decoder cases 的 205/474 数字由母仓 `docs/UPSTREAM-TOML-TEST.md` 自证记录（该文如实注记：完整输出无入库载体、上游语料不在仓内、无法复算；记录载体与脚本归属均以该文为准）；母仓 `scripts/run-toml-test.ps1` 是纯执行器（仅 `cargo build` + `go run toml-test` + exit，无任何写入命令），不产生该记录；门禁为记录载体：脚本与记录在母仓、无 CI job 执行——六仓拆分后母仓根无 Cargo.toml，脚本在母仓原位不可执行；可执行入口的迁移/重建待总指挥决策，2026-08-14 波 2 处置）；本仓 ci-typescript.yml/release.yml 无运行它的 job，本仓发布路径不含该门禁；上游版本变更必须单独审计。本仓对应机制为 conformance 门禁的 toml-v1 套件（18 套 / 519 cases 之一）与 TS-Rust 差分 harness。semantic edit 不会舍入 NaN payload、亚纳秒时间或非整分钟 offset 来伪造成功。
...
`hcl.native@1`/`hcl.tfvars@1` 的 parse/query/project/edit 全程不求值：无 variable/function/template 求值与展开，`hcl.expression@1` 只承载语法事实、永不执行，无 application schema 与 Terraform/cty 语义。formation 只消费调用方提供的完整文档字节，不访问文件、网络、registry 或环境。表达式/模板/heredoc depth、number digits、item/label/attribute counts 与 recovery regions 等全部尺寸算术在分配前 checked，limit 失败绝不伪装成空 body、截断表达式或缩短查询。恢复文档可查询、不可 project/materialize/commit；`hcl.canonical-document@1` materialization 生成字节必先重解析并逐节点比较闭包语义，失败返回无目标 Document、无 partial bytes、无 partial provenance。极限输入（expression depth、template/heredoc size、number digits、body nesting 与 item counts）的覆盖在 ts 侧为 conformance 门禁的 hcl-v1 套件与 TS-Rust 差分 harness（本仓 src/hcl/ 无独立 hardening 测试文件）；「无 panic」是 Rust 参考实现侧的声称（见 consema-rs 仓 SECURITY.md hcl_hardening.rs）——TS 无 panic 概念，对应约束为上述资源上限检查在分配前执行、超限以 typed error 原子失败、无未捕获异常与无界分配。
