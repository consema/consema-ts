...
if ($caseCount -ne 68) {
    Write-Error "differential case file has $caseCount cases, want exactly 68 (frozen count, G66)"
    exit 1
}
...
if ($OutDir -eq '') {
    $OutDir = Join-Path $targetDir "ts-differential-parity-$nonce"
}
...
$logDir = Join-Path $env:TEMP "consema-ts-parity-$nonce"
