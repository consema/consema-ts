...
**包根可导入符号**（`@consema/consema` 直接 import）：`parseDocument` / `Document` / `formatFamilies` / `profiles` / `queryDomains` / `formatOperationRegistry` / `FormatProfile` / `convert*` / `ConversionResult`（以及 core / graph / protocol 域记录）。签名中的类型名分两类（波 4 R23 修正，2026-08-15；`typescript/src/index.ts` 的 `export *` 面为权威，本段与其一致——export 语句分布于 :20-42 与 :47-48 行，行号可能漂移，以 export 语句分布为锚）：
...
- **包根不 re-export 的类型**：document / json / toml / yaml 等家族模块内的类型名（`ProfileId`、`JsonDocument`、`ProjectionRequest`、`MaterializationRequest`、`JsonQueryResult` 等）不随包根 re-export，且 npm `exports` 仅暴露包根（tarball 内容为 dist + LICENSE 与 npm 自动附带的 package.json、包 README——不含任何 `.ts` 源码，见 RELEASING.md §4）——模块内类型**不可从包导入**，实例一律经包根入口获得（如 `profiles()` 返回 `FormatProfile[]`、取其 `.profile()` 得 `ProfileId`）；需要这些概念的注解时用包根 re-export 面的对应类型（W3-43 分列注记，2026-08-14）：
...
- **跨语言一致性如何保证？** 18 套语言无关 conformance suite 共 519/519 cases 由规范仓维护、五仓共享（聚合 digest `cfd6e296…` 由规范仓 manifest 记录维护；断言机制各仓不同——rs 为 vendored DIGEST 比对、go/py/kt 为 provision 时 manifest sha256 断言、ts 为 runner 对 provisioned manifest 的运行时比对，ts CI 不 provision manifest、该比对为 documented skip，见 R10/W4-13）；CI 多仓 checkout 跑 conformance runner 与 TS-Rust 差分门禁（byte parity / normalized differential / protocol-exchange）。
- **兼容承诺？** 语义化版本；`check-version-consistency` 门禁断言 README 版本行与 `package.json` 一致；`tsc --noEmit` strict 在 `src/` 全树零诊断（`examples/`、`typescript/quickstart.ts` 与 `tools_*.mjs` 不在 tsconfig include 内，不经 tsc——quickstart.ts 是已入库、被 CI examples job 实际执行的示例文件，属运行时而非类型检查面）；兼容与支持政策见 RFC 0020。
